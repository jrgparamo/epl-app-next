...
async function standingsThroughMatchday(matchday, currentUser) {
  try {
    ...
    const userIds = grouped.map((g) => g.userId);
    const [correctCounts, predictedCounts, users] = await Promise.all([
      prisma.userPoints.groupBy({
        by: ["userId"],
        where: {
          userId: { in: userIds },
    ...
    if (currentUser && !leaderboard.some((e) => e.user_id === currentUser.id)) {
      const [meAgg, meCorrect, mePredicted, meUser] = await Promise.all([
        prisma.userPoints.aggregate({
          where: { userId: currentUser.id, matchId: { in: matchIds } },
          _sum: { pointsEarned: true },
        }),
        prisma.userPoints.count({
          where: {
    ...
    console.error("GET /api/leaderboard?matchday error:", error);
    return NextResponse.json(
      { error: "Failed to fetch matchday leaderboard" },
      { status: 500 },
    );
  }
}
